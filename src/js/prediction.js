...
class PredictionEngine {
  ...
  // ── Trade timing breakdown (minute-by-minute projection) ─────────────────
  // Returns projected price levels for each minute from 0 to maxMinutes
  generateTradeTimeline(signal, candles1m, maxMinutes = 45) {
    ...
    const atr1m     = this._atr(candles1m.slice(-14), 14);

    // Average price move per minute from recent data
    const recentMoves = [];
    for (let i = 1; i < Math.min(20, candles1m.length); i++) {
      const move = candles1m[candles1m.length - i].close -
                   candles1m[candles1m.length - i - 1].close;
      recentMoves.push(Math.abs(move));
    }
    const avgMovePerMin = recentMoves.reduce((a, b) => a + b, 0) / recentMoves.length || atr1m;

    // Momentum direction (slope of last 10 mins)
    const slope     = this._linearRegressionSlope(candles1m.slice(-10).map(c => c.close));
    const biasedDir = isBuy ? 1 : -1;
    const momentumBoost = (slope > 0 && isBuy) || (slope < 0 && !isBuy) ? 1.2 : 0.8;

    // Generate timeline points
    const timeline = [];
    let simulatedPrice = entry;
    let hitSL = false, hitTP = false;

    for (let min = 0; min <= maxMinutes; min++) {
      if (hitSL || hitTP) {
        const status = hitSL ? 'sl' : hitTP ? 'tp' : 'open';
        timeline.push({
          minute: min, price: simulatedPrice,
          status, slDistance: null, tpDistance: null,
        });
        continue;
      }

      // Project price with momentum + mean reversion
      const noise     = (Math.random() - 0.48) * avgMovePerMin;
      const momentum  = biasedDir * avgMovePerMin * 0.15 * momentumBoost;
      // Mean reversion toward midpoint of SL/TP
      const midpoint  = (sl + tp1) / 2;
      const pullback  = (midpoint - simulatedPrice) * 0.01;
      simulatedPrice += momentum + noise + pullback;

      // Check SL/TP
      if (isBuy && simulatedPrice <= sl)  { simulatedPrice = sl;  hitSL = true; }
      if (isBuy && simulatedPrice >= tp1) { simulatedPrice = tp1; hitTP = true; }
      if (!isBuy && simulatedPrice >= sl)  { simulatedPrice = sl;  hitSL = true; }
      if (!isBuy && simulatedPrice <= tp1) { simulatedPrice = tp1; hitTP = true; }

      const pip        = window.marketData?.getPip?.(signal.symbol) || 0.0001;
      const slDist     = ((simulatedPrice - sl) / pip * (isBuy ? 1 : -1)).toFixed(0);
      const tpDist     = ((tp1 - simulatedPrice) / pip * (isBuy ? 1 : -1)).toFixed(0);

      timeline.push({
        minute:    min,
        price:     parseFloat(simulatedPrice.toFixed(5)),
        status:    hitSL ? 'sl' : hitTP ? 'tp' : 'open',
        slDist:    parseInt(slDist),
        tpDist:    parseInt(tpDist),
        pnlR:      parseFloat(((isBuy ? simulatedPrice - entry : entry - simulatedPrice) /
                               Math.abs(entry - sl)).toFixed(2)),
      });
    }

    // Estimate most likely outcome minute
    const tpMinute = timeline.find(t => t.status === 'tp')?.minute;
    const slMinute = timeline.find(t => t.status === 'sl')?.minute;
    const exitEst  = tpMinute || slMinute;

    return {
      timeline:    timeline.slice(0, maxMinutes + 1),
      predictedExit: exitEst ? `~${exitEst} min` : `>${maxMinutes} min`,
      hitTP:       !!tpMinute,
      hitSL:       !!slMinute,
      avgMovePerMin: parseFloat(avgMovePerMin.toFixed(5)),
    };
  }
